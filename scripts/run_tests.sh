...
# ============================================================
# Lab 9 — Class System (Inheritance) Test Runner
# ============================================================
...
SRC_DIR="$ROOT_DIR/assignments/classes"
TEST="$ROOT_DIR/tests/ClassesTest.java"
...
compile_all() {
    mkdir -p "$BUILD_DIR"
    echo -e "${DIM}[1/3] Character / Warrior / Mage / Rogue / Paladin компайл хийж байна...${RESET}"
    javac -d "$BUILD_DIR" "$SRC_DIR"/*.java 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}  ✗ Класс компайл алдаатай!${RESET}"
        exit 1
    fi
    echo -e "${GREEN}  ✓ Бүх класс компайл амжилттай${RESET}"

    echo -e "${DIM}[2/3] ClassesTest.java компайл хийж байна...${RESET}"
...
main() {
    echo -e "${BOLD}${MAGENTA}"
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║  🐉 Lab 9 — Class System Inheritance Grader  ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${RESET}"
    ...
    # AI Detection — only the student-edited files
    echo -e "${DIM}AI Detection шалгаж байна...${RESET}"
    for f in Warrior Mage Rogue Paladin; do
        if [ -f "$SRC_DIR/$f.java" ]; then
            python3 "$SCRIPT_DIR/ai_detector.py" "$SRC_DIR/$f.java"
        fi
    done
}
